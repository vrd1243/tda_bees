...
Finally, we also provide the following notebooks, which are not essential to the pipeline:

1. code/visuals_for_publication.ipynb: This can be used to generate all the changepoint related figures in the paper. The appropriate input files and changepoint values generated by our scripts need to be manually provided.

2. code/dot_whiskers_chart.ipynb: Used for generate the t_{lag} plot (Fig. 8).
